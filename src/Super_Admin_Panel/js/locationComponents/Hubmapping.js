...
const Hubmapping = ({ state }) => {
  ...
  useEffect(() => {
    ...
    var config = {
      ...
      url: Degital_Ocean_flag
        ? "https://e-deshdelivery.com/api/v1.1/Hub/GetAllHub"
        : "http://test.e-deshdelivery.com/api/v1.1/Hub/GetAllHub",
      headers: {
    ...
    axios(config)
      .then((res) => {
         console.log("getall hub", res);
        setroothub(res?.data?.data);
        setIsLoading(false);
      })
  ...
  console.log("get all hub",roothub)
...
//   const dropdownOptions = [{ id: 1, val: "option1" }, { id: 2, val: "option2" }, { id: 3, val: "option3" }];
  ...
  function handleDropdownChange(record, selectedOption) {
    const newRowData = {
      ...
      // IsActive:0, // Use a suitable identifier for your row
      HubId: parseInt(selectedOption),
  ...
  
  {
    ...
    render: (text, record) => (
      <select onChange={(e) => handleDropdownChange(record, e.target.value)}>
        {roothub?.map((option, index) => (
          <option key={index} value={option.hubId}>
            {option.hubName}
          </option>
        ))}
      </select>
...
//     render: (text, record) => (
//       <div>
//         <label htmlFor={`dropdown-${record.locationId}`} className="form-label">Select Hub</label>
//         <select
//           id={`dropdown-${record.locationId}`}
//           className="form-select"
//           onChange={(e) => handleDropdownChange(record, e.target.value)}
//         >
//           {roothub.map((option, index) => (
//             <option key={index} value={option.hubId}>
//               {option.hubName}
...
//   },
  ...
  function Submit(e){
    ...
    let data = JSON.stringify(selectedValues);
      ...
      let config = {
        ...
        url: Degital_Ocean_flag
          ? "https://e-deshdelivery.com/api/v1.1/Location/UpdateLocations"
          : "http://test.e-deshdelivery.com/api/v1.1/Location/UpdateLocations",
        headers: {
          "Content-Type": "application/json",
        },
        data: data,
  ...
  return (
    <div className="bodydiv">
      ...
      <div className="content">
        ...
        <button onClick={(e)=>Submit(e)} className="btn btn-primary mb-3">Submit</button>
