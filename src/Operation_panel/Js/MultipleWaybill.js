...
const MultipleWaybill = () => {
  ...
  const csvData = [["Reference Number"], [""]];
  ...
  let csv_upload_filebutton_function = (e) => {
    ...
    if (logingInformation_LocalStore.token != undefined) {
      ...
      // console.log("basefiledata==", data);
      //console.log("fileName: ", file_name);
      var config = {
        method: "post",
        url: Degital_Ocean_flag
          ? "https://e-deshdelivery.com/SqlCommand/Faltu_kaj" +
            "?company_name=" +
            company_name
          : "/SqlCommand/Faltu_kaj" + "?company_name=" + company_name,
        headers: {
      ...
      axios(config)
        .then(function (response) {
          ...
          setCsvUpload(response?.data?.message?.response);
          setIsLoading(false);
          // setCsvUpload(response?.data?.message?.response);
            ...
            <div className="col-12 pt-4" id="">
              ...
              <div>
                <div id="no-more-tables">
                  <CSVLink
                    data={csvUpload}
                    className="btn btn-dark btn-sm me-2 fw-bold px-4 rounded mb-2"
                  >
                    Export csv
                  </CSVLink>
                  <CSVLink
                    ...
                    data={csvData}
                    className="btn btn-info btn-sm me-2 fw-bold px-4 rounded mb-2"
                  >
                    CSV Template
                  </CSVLink>
                  <table
                  ...
                  >
                    ...
                    >
                      <tr className="text-dark" style={{ border: "none" }}>
                        <th>Product Waybill</th>
                        <th>Consignee Name</th>
                        <th>Reference NO</th>
                      </tr>
                    </thead>
                    <tbody className="text-center border border-dark">
                      {csvUpload.map((singleData) => {
                        return (
                          <tr key={singleData.product_waybill}>
                            <td data-title="Product Waybill">
                              {singleData.product_waybill}
                            </td>
                            <td data-title="Consignee Name">
                              {singleData.consignee_name} cn
                            </td>
                            <td data-title="Reference NO">
                              {singleData.reference_no}rn
                            </td>
                          </tr>
                        );
                      })}
...
export default MultipleWaybill;
