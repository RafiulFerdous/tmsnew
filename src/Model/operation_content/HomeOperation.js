  ...
  useEffect(() => {
    ...
    allfilterproductdata.map((single) => {
      ...
      let obj = {
        ...
        // "All Step Count": single.all_step_count_list[0]?.status_date,
        // "All Step Count": single?.all_step_count_list[1]?.status_date,
      };

      single.all_step_count_list.map((e) => {
        const newobj = {
          ["attempt " + e.id]: e.id,
          // ["step status " + e.id]: e.status,
          ["attempt date " + e.id]: e.status_date,
        };
        result = { ...result, ...newobj };
      });
      // csv.push(obj);
      let finalObj = { ...obj, ...result };
      csv.push(finalObj);
    ...
    <>
      <Modal2 show={show} onHide={handleClose}>
        <Modal2.Header closeButton>
          <Modal2.Title>Step Count</Modal2.Title>
        </Modal2.Header>
        {stepData.map((e) => (
          <div className="p-3" key={e.id}>
            <p>Step : {e.id}</p>
            <p>status : {e.status}</p>
            <p>Date : {e.status_date}</p>
          </div>
        ))}
        <Modal2.Footer>
              ...
              >
                ...
                >
                  <tr className="text-dark" style={{ border: "none" }}>
                    ...
                    <th scope="col">Current Status</th>
                    <th scope="col">Entry Date</th>
                    <th scope="col">Pickedup Date</th>
                    <th scope="col">Date</th>
                    <th scope="col">Reason</th>
                    <th scope="col">SLA Status</th>
                    <th scope="col">Paid/Unpaid</th>
                    <th scope="col">Open/Close</th>
                    <th scope="col">Nextday/Regular</th>
                    <th scope="col">Total Attempt</th>
                    <th scope="col">All Step Count</th>
                  </tr>
                </thead>
                {/*Table head*/}
                {/*Table body*/}
                    ...
                    current.map((single_message) => {
                      ...
                      return (
                        <tr className="">
                          ...
                          <td data-title="Entry Date">
                            {single_message.pickedup_date &&
                              single_message.pickedup_date.split("T")[0]}
                          </td>
                          ...
                          <td data-title="SLA Status">{sla}</td>
                          <td data-title="Paid/Unpaid">
                            {single_message.paid_or_unpaid}
                          </td>

                          <td data-title="Paid/Unpaid">
                            {single_message.openBox_or_CloseBox}
                          </td>

                          <td data-title="Paid/Unpaid">
                            {single_message.nextDay_or_Regular}
                          </td>
                          ...
                          >
                            <button
                              ...
                              onClick={() =>
                                handleStepCount(
                                  single_message?.all_step_count_list
                                )
                              }
                            >
                              See Step
                            </button>
                            {/* {single_message?.all_step_count_list?.map((e) => (
                              <div key={e.id}>
                                <p>Step : {e.id}</p>
                                <p>status : {e.status}</p>
                                <p>Date : {e.status_date}</p>
